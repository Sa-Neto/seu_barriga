...
const TOKEN = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjEwMDAwIiwibmFtZSI6IlVzZXIjMSIsIm1haWwiOiJtYWlsQHVzZXIxLmNvbSJ9.qP02b2FZNMsUSFb-XQZUgxAvZ9L4JCM7pfdAvVKRRN4'
...
describe('Ao salvar uma transferência válida...',() => {
  ...
  test('Deve retornar o status 201 e os dados da transfêrencia', () => {
    return request(app).post(MAIN_ROUTE)
      .set('authorization', `bearer ${TOKEN}`)
      .send({ description: 'Regular Transfer', user_id: 10000, acc_ori_id: 10000, acc_dest_id: 10001, ammount: 100, date: new Date()})
      .then(async(res) => {
...
describe('Ao tentar salvar uma transferência inválida...',() =>  {
  ...
  test('Não deve inserir sem conta de origem e destino forem as mesmas',() => template({acc_dest_id:10000}, 'Não é possivel transferir de uma conta para ela mesma'));
  test('Não deve inserir sem conta de pertencerem a outro usuário',() => template({acc_ori_id: 10002}, 'Conta #10002 não pertence ao usuário' ));
})
...
describe('Ao alterar uma transferência válida...',() => {
  ...
  test('Deve retornar o status 200 e os dados da transfêrencia', () => {
